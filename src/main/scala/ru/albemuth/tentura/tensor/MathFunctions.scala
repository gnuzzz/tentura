...
object MathFunctions {
  ...
  def relu[T: ClassTag](vector: Vector[T], result: Vector[Float]): Vector[Float] = {
    MathKernel.vector_r(reluf, vector, result)
  }

  def relu[T: ClassTag](vector: Vector[T]): Vector[Float] = {
    MathKernel.vector(reluf, vector)
  }

  def relu(vector: Vector[Double], result: Vector[Double]): Vector[Double] = {
    relud(vector, result)
  }
  ...
  def relud[T: ClassTag](vector: Vector[T], result: Vector[Double]): Vector[Double] = {
    MathKernel.vector_r(relud, vector, result)
  }

  def relud[T: ClassTag](vector: Vector[T]): Vector[Double] = {
    MathKernel.vector(relud, vector)
  }

  def relu[T: ClassTag](matrix: Matrix[T], result: Matrix[Float]): Matrix[Float] = {
    MathKernel.matrix_r(reluf, matrix, result)
  }

  def relu[T: ClassTag](matrix: Matrix[T]): Matrix[Float] = {
    MathKernel.matrix(reluf, matrix)
  }

  def relu(matrix: Matrix[Double], result: Matrix[Double]): Matrix[Double] = {
    relud(matrix, result)
  }
  ...
  def relud[T: ClassTag](matrix: Matrix[T], result: Matrix[Double]): Matrix[Double] = {
    MathKernel.matrix_r(relud, matrix, result)
  }

  def relud[T: ClassTag](matrix: Matrix[T]): Matrix[Double] = {
    MathKernel.matrix(relud, matrix)
  }
